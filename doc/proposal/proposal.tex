...
In Shotton et. al \cite{shotton} the state of the art (SOTA) methods were described as \bold{sparse baseline} and \bold{Tiny-image baseline}. 

For \bold{sparse baseline.} technique, local feature detection and descriptors are used. These descriptors are used to build a 3D point cloud with RGB-D training images and matched at test time with the current image. For more robustness RANSAC \cite{Henry10rgbdmapping} for the matching is used. \cite{DBLP:conf/bmvc/GeeM12} 

\bold{tiny-image baseline.} This baseline represents a whole-image matching approaches. Therfore the training images are down-sampled and blurred with a Gaussian kernel. At test time, brute-force matching for matching is used.
